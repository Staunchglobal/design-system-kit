...
export function useElementTracking(target: Element | null): DOMRect | null {
  ...
  return React.useMemo(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- `tick` intentionally participates only to force a re-measure on observer/listener events; it carries no data of its own
  }, [target, tick])
}
